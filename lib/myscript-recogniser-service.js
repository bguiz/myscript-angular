  ...
  function MyscriptRecogniserServiceProvider() {
    ...
    */
    function MyscriptRecogniserServiceFactory($q) {
      ...
      function recognise(myscriptRecogniseType, strokes) {
        var recogniser = MyscriptRecogniser();

        // Use `q.defer()` instead of `q()` to support the form that
        // is bundled with older versions of AngularJs
        var deferred = $q.defer();
        var promise = deferred.promise;
        recogniser.recognise(myscriptRecogniseType, strokes, function onRecongise(err, data) {
          if (!!err) {
            deferred.reject(err);
          }
          else {
